...
export async function uploadAvatar(req: Request, res: Response): Promise<void> {
  try {
    ...
    console.log('📸 [UPLOAD] Starting avatar upload for user:', userId);
      ...
      if (user.avatarUrl) {
        ...
        if (oldPublicId) {
          ...
          try {
            ...
            console.warn('⚠️  [CLOUDINARY] Failed to delete old avatar:', deleteError.message);
          }
        }
      }
...
export async function deleteAvatar(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!publicId) {
      console.error('❌ [DELETE] Failed to extract public_id from URL:', user.avatarUrl);
    ...
    try {
      ...
      res.status(500).json({
        error: 'Failed to delete avatar from Cloudinary',
        details: cloudinaryError.message || 'Unknown Cloudinary error',
      });
    }
  } catch (error: any) {
