...
export const saveToCache = async <T>(
	...
	try {
		...
		console.log({ msg: "❌ Cache error :", Error: error });
...
export const invalidateCache = async (pattern?: string) => {
	try {
		if (pattern) {
			...
			console.log(
				`🗑️ Invalidation de ${result.deletedCount} entrées avec pattern: ${pattern}`,
			);
			...
			const result = await ApiCache.deleteMany({});
			console.log(
				`🗑️ Invalidation de tout le cache: ${result.deletedCount} entrées`,
			);
			return result.deletedCount;
		}
	} catch (error) {
